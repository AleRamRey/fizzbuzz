...
app.get("/", (request, response) => {
    response.json({message: "Explorers and FizzBuzz Api welcome!"});
});
...
app.get("/v1/fizzbuzz/:number", (request,response) =>{
    ...
    const fizzbuzzTrick = ExplorerController.getFizzbuzz(number);
    response.json({score: request.params.number, trick: fizzbuzzTrick});
});

app.listen(port, () => {
    console.log(`Explorers and FizzBuzz API in localhost:${port}`);
});